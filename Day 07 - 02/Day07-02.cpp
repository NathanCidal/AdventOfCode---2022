...
class DirLinkedTree{
	...
	public:
		...
		int buscaMenorSizeEncontrado(Node * aux, int minimumFound, int minimumWanted){
			if(aux == nullptr) return minimumFound;
			
			if(aux->size >= minimumWanted && aux->size < minimumFound){
				minimumFound = aux->size;
			}

			for(int i = 0; i < aux->childs.size(); i++){
				if(aux->childs[i]->type == "dir"){
					minimumFound = buscaMenorSizeEncontrado(aux->childs[i], minimumFound, minimumWanted);
				}
			}
...
int main(){		
	...
	//Find how much free Space we have
	long long int totalDisk = 70000000;
	long long int avaliable = 70000000 - tree->returnHead()->size; 

	long long int minimumWanted = 30000000 - avaliable;	//Minimum Size above we want
	cout << "Minimum Size Wanted = " << minimumWanted << endl;

	cout << "Minimum Found = " << tree->buscaMenorSizeEncontrado(tree->returnHead(), totalDisk, minimumWanted) << endl;
